class Internee < ApplicationRecord

	########## Validation ############
	validates :internee_code, 			:uniqueness => { scope: :company_id }
  ...
  def make_employee
    if self.is_converted == true
      ...
      employee.prev_employee_code     = self.internee_code.to_i.to_s
      employee.save
    end
  end

  def self.location_related_employee(internees_list, location_id)
    internees = internees_list.where(:location_id => location_id)
    return internees
  end

  def self.branch_related_employee(internees_list, branch_id)
    internees = internees_list.where(:branch_id => branch_id)
    return internees
  end

  def self.department_related_employee(internees_list, department_id)
    internees = internees_list.where(:department_id => department_id)
    return internees
  end

  def self.designation_related_employee(internees_list, designation_id)
    internees = internees_list.where(:designation_id => designation_id)
    return internees
  end

  def self.job_title_related_employee(internees_list, job_title_id)
    internees = internees_list.where(:job_title_id => job_title_id)
    return internees
  end

  def self.grade_related_employee(internees_list, grade_id)
    internees = internees_list.where(:grade_id => grade_id)
    return internees
  end

  def self.salary_unit_related_employee(internees_list, salary_unit_id)
    internees = internees_list.where(:salary_unit_id => salary_unit_id)
    return internees
  end

  def self.cost_center_related_employee(internees_list, cost_center_id)
    internees = internees_list.where(:cost_center_id => cost_center_id)
    return internees
  end
