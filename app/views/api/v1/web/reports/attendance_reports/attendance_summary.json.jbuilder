		...
		if not employee.nil?
			if employee.is_active == true && employee.salary_exempted == false && employee.excluded_from_reports == false
				...
				if employee.approval_base_overtime == true
					json.over_time 				depatment_wise_attendances.where(:employee_id => employee.id).sum(:approved_overtime)	
				else
					json.over_time 				depatment_wise_attendances.where(:employee_id => employee.id).sum(:over_time_hours)	
				end
				json.cpl_earned 			depatment_wise_attendances.where(:employee_id => employee.id).sum("no_of_cpl")
				json.off_days 				depatment_wise_attendances.where(:employee_id => employee.id).sum("off_days_payment_days")
				json.worked_days 			working_days.to_f.round(2)
				json.total_pay_days 	(working_days.to_f.round(2) + depatment_wise_attendances.where(:employee_id => employee.id).sum("off_days_payment_days")).round(2) + arrear_days.to_f.round(2)
				json.leave_avalied 		LeaveRequest.where("start_date >= ? AND end_date <= ?", params[:start_date].to_date, params[:end_date].to_date).where(:is_cancelled => false, :request_status => "Availed", :employee_id => employee.id).sum(:request_count)
				json.leave_deduction 	LeaveRequest.where("start_date >= ? AND end_date <= ?", params[:start_date].to_date, params[:end_date].to_date).where(:is_cancelled => false, :request_status => "System Deducted", :employee_id => employee.id).sum(:request_count).round(2)
				json.cpl_avalied 			LeaveRequest.where("start_date >= ? AND end_date <= ?", params[:start_date].to_date, params[:end_date].to_date).where(:is_cancelled => false, :request_status => "Availed", :employee_id => employee.id, :leave_type_id => leave_type_ids).sum(:request_count)
