...
json.roster_list @employees.each do |employee|
	...
	json.time_slot_id 					@time_slot.id
	@employee_rosters = EmployeeRoster.where(:employee_id => employee.id, :roster_date => @complete_date_range).order('id DESC')
	if @employee_rosters.count == 0
		json.roster_status 				"No Rest Day Exist on selected dates"
	else
		# roster_dates = @employee_rosters.where(:is_rest_day => true).collect(&:roster_date).to_a.map{|x| x.to_date.strftime("%A, %B %-d, %Y")}
		# if roster_dates.count > 0
		# 	json.roster_status 				"Rest Day Already Exist on these dates #{roster_dates.join(' | ')}"
		# else
		# 	json.roster_status 				"No Rest Day Exist on selected dates"
		# end
		json.roster_status 				"-"
